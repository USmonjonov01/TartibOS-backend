...
export async function generateRoutinePlan({ goal, existingRoutines = [], dailyHours }) {
    ...
    // "low" o'rniga "high": bu funksiya ham foydalanuvchining erkin matnini
    // (izoh, mavjud odatlar, bo'sh vaqt) chindan "o'ylab" birlashtirishi
    // kerak — aiRoadmap.js'dagi bilan bir xil sabab.
    const parsed = await generateJson({ systemPrompt: SYSTEM_PROMPT, userText, thinkingLevel: "high" });
